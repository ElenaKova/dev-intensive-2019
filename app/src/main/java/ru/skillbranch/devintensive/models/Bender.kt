...
class Bender(var status: Status = Status.NORMAL, var question: Question = Question.NAME) {
    ...
    enum class MessageError(val message: String){
        MESSAGE_NAME("Имя должно начинаться с заглавной буквы"),
        MESSAGE_PROFESSION("Профессия должна начинаться со строчной буквы"),
        MESSAGE_MATERIAL("Материал не должен содержать цифр"),
        MESSAGE_BDAY("Год моего рождения должен содержать только цифры"),
        MESSAGE_SERIAL("Серийный номер содержит только цифры, и их 7"),
        MESSAGE_SUCCESS("Отлично - ты справился\n На этом все, вопросов больше нет")
    }
}